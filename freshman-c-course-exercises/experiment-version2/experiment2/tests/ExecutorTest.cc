...
//测试倒车变过去再拐弯
TEST(Executor, should_return_x_3_y_1_given_command_is_FMLBMLMFM_and_facing_is_N)
{
    // given
    ExecutorInitialize((Pose){.pos = {.x = 0, .y = 0}, .heading = NORTH});
    // when
    ExecutorExecCommands("FMLBMLMFM"); //(0,0),N->(0,2),N->(0,3),W->(2,3),W->(3,3),N->(3,1),N
    // then
    Pose pose = ExecutorQuery();
    ASSERT_EQ(pose.pos.x, 4);
    ASSERT_EQ(pose.pos.y, 1);
    ASSERT_EQ(pose.heading, NORTH);
}
